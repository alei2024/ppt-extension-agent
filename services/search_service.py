...
class SearchService:
    ...
    def search_arxiv(self, query: str, max_results: int = 3) -> List[Dict]:
        """
        在Arxiv中搜索相关论文（优化：处理查询词格式，自动翻译中文为英文）
        ...
        try:
            ...
            if has_chinese:
                # 尝试翻译为英文
                try:
                    logger.info(f"检测到中文查询: {query}，尝试翻译为英文用于Arxiv搜索...")
                    prompt = f"Translate the following academic search query to English. Output only the translated text, no other words.\n\nQuery: {query}"
                    ...
                    logger.warning(f"翻译失败: {e}，将尝试使用原始查询的非中文部分")
            
            logger.info(f"在Arxiv搜索: {search_query[:50]}...")
            
            # 2. 构造Arxiv查询
            # 如果翻译失败且仍含中文，尝试提取非中文部分
            if bool(re.search(r'[\u4e00-\u9fa5]', search_query)):
                cleaned = re.sub(r'[\u4e00-\u9fa5]', '', search_query).strip()
                if cleaned and len(cleaned) >= 3:
                    search_query = cleaned
                # 如果清理后太短，Arxiv可能无法处理，但我们仍尝试（或直接返回空）
            ...
            for result in search.results():
                results.append({
                    ...
                    "relevance": self._calculate_relevance(query, result.title, result.summary) # 用原查询计算相关度可能不准，但这里先这样
                })
    ...
    def multi_source_search(
        ...
        """
        if sources is None:
            sources = ["arxiv", "semantic_scholar", "crossref", "openalex"]  # 启用多源搜索，Wikipedia已禁用
